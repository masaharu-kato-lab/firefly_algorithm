...
def betaStep(_A, _B, gamma): #move b to a
    ...
    for i in range(len(C)):
        if A[i] == B[i] or random.random() < beta:
            if A[i] not in C:
                C[i] = A[i]
                remains.remove(A[i])
        else:
...
if __name__ == "__main__":
    ...
    argp.add_argument('-t' , '--n_iteration'     , type = int  , required=True, help = 'number of iterations')
    argp.add_argument('-g' , '--gamma'           , type = float, required=True, help = 'firefly algorithm gamma')
    argp.add_argument('-a' , '--alpha'           , type = int  , required=True, help = 'firefly algorithm alpha')
    argp.add_argument('-n' , '--n_fireflies'     , type = int  , required=True, help = 'number of fireflies')
    argp.add_argument('-e' , '--dist_penal_coef' , type = float, required=True, help = 'distance penalization coeficient')
    argp.add_argument('-s' , '--seed'            , type = int  , default =None, help = 'seed value (Randomly determined if not specified)')
