...
class firefly:
    ...
    def evaluate(self, x):
        ...
        for element in list(point_time.values()):
            uncertainties.append(1 - math.exp(-0.001279214 * (end_patrol_time - element)))

        ave_uncertainty = sum(uncertainties) / len(uncertainties)

        return ave_uncertainty, battery, solution
